...
def animal_facts():
    ...
    context = {
        ...
        "animal_to_fact" : animal_facts_keys,
...
def image_filter():
    ...
    if request.method == 'POST':        
        ...
        user_file = request.values.get('filter_type')
        ...
        new_file = save_image(image, user_file)
        # Call `apply_filter()` on the file path & filter type
        image_url = f'./static/images/{image.filename}'
        apply_filter(new_file, user_file)
...
def gif_search():
    ...
    if request.method == 'POST':
        ...
        user_query = request.values.get('search_query')
